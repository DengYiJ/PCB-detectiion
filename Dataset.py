...
class MyDataset(Dataset):#transform=True表示进行变换，会把他变成张量
    def __init__(self, root_dir, transform=None, train=True, test_size=0.1667, random_state=42, num_anchors=6, num_classes=7,rotate_angle=None):
    ...
    def __getitem__(self, item):
        ...
        # 添加填充步骤
        padded_image = Image.new('RGB', (1024, 1024), (114, 114, 114))
        padded_image.paste(image, ((1024 - new_w) // 2, (1024 - new_h) // 2))
    ...
    def convert_to_model_input(self, objects,img_width,img_height):
        ...
        for i, obj in enumerate(objects):
            ...
            # 设置分类标签（one-hot 编码）
            class_idx = name_to_index.get(name, 0)  # 默认是最末尾的索引
            #print(f"Object {i}: name={name}, class_idx={class_idx}")  # 调试信息
            # 清除背景类标记
            y_batch[i, 0] = 0.0  # 重要！移除默认背景标记
            y_batch[i, class_idx] = 1.0  # 设置实际类别
            assert class_idx != 0, "检测到背景类被错误标注"
